...
else if($dopost=='save')
{
    ...
    if(!$dsql->ExecuteNoneQuery($query))
    {
        ...
        ShowMsg("把数据保存到数据库主表 `#@__archives` 时出错，请把相关信息提交给炫乐科技官方。".str_replace('"','',$gerr),"javascript:;");
        exit();
    }
    ...
    if(!$dsql->ExecuteNoneQuery($query))
    {
        ...
        $dsql->ExecuteNoneQuery("DELETE FROM `#@__archives` WHERE id='$arcID'");
        $dsql->ExecuteNoneQuery("DELETE FROM `#@__arctiny` WHERE id='$arcID'");
        ShowMsg("把数据保存到数据库附加表 `{$addtable}` 时出错，请把相关信息提交给炫乐科技官方。".str_replace('"','',$gerr),"javascript:;");
        exit();
    }
