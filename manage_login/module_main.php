...
$mdurl = $updateHost.'炫乐科技/module_'.$cfg_soft_lang.'/modulelist.txt';
...
function SendData($hash = '',$type = 1)
{
    if(!empty($hash)){
        ...
        $fp = fsockopen('www.炫乐科技.com',80,$errno,$errstr,30); 
        if(!$fp)
        {
            return FALSE;
        }else{ 
            fputs($fp, "POST http://www.炫乐科技.com/plugin.php HTTP/1.1\r\n"); 
            fputs($fp, "Host: www.炫乐科技.com\r\n"); 
...
{
	$model_remote_url = $updateHost.'炫乐科技/module_'.$cfg_soft_lang.'/'.$hash.'.xml';
